...
void aes_tiling(uint8_t* local_key, uint8_t* buf, uint8_t* buf_out, uint8_t* key_out) {
...
void acc_3( uint8_t* local_key, uint8_t buf[UNROLL_FACTOR][BUF_SIZE/UNROLL_FACTOR], uint8_t buf_out[UNROLL_FACTOR][BUF_SIZE/UNROLL_FACTOR], uint8_t key_out[UNROLL_FACTOR]) {

        int j;
        unroll_loop: for (j=0; j<UNROLL_FACTOR; j++) {
	    aes_tiling(local_key[j], buf[j], buf_out[j], &key_out[j]);
	}
}
...
struct out aqed_top (uint8_t bmc_in[UNROLL_FACTOR][BUF_SIZE/UNROLL_FACTOR], int orig, int dup) {

struct out o2;
int j,k;
uint8_t buf[UNROLL_FACTOR][BUF_SIZE/UNROLL_FACTOR];
uint8_t key[UNROLL_FACTOR];
uint8_t key_in[UNROLL_FACTOR][32];
...
void wrapper(){
	uint8_t bmc_in[UNROLL_FACTOR][BUF_SIZE/UNROLL_FACTOR];
	int orig, dup;
