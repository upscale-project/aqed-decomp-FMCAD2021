...
struct out aqed_top (uint8_t bmc_in[BUF_SIZE], int orig, int dup, bool orig_batch_idx, bool dup_batch_idx, bool batch) {
...
for(j=0;j<BATCH_SIZE;j++){//serializer-deserializer
	aqed_in(&bmc_in[j*IN_SIZE], orig, dup, orig_batch_idx, dup_batch_idx, batch);
	if(j==dup) break;
}
