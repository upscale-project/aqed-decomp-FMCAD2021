...
struct out aqed_top (uint8_t bmc_in[16], struct aes256_context *ctx, int orig, int dup, bool orig_batch_idx, bool dup_batch_idx, bool batch) {
...
bool batch=0;
	while(1){
		struct out o3 = aqed_top(bmc_in[batch], &ctx, orig, dup, orig_batch_idx, dup_batch_idx, batch);
